...
VL_NON_TENSOR_KEYS_FOR_CRITIC = ("multi_modal_inputs", "multi_modal_data")


def is_vl_batch_for_critic(data) -> bool:
    """Return True if the batch carries VL-specific fields that text critics cannot use."""
    if any(k in data.non_tensor_batch for k in VL_NON_TENSOR_KEYS_FOR_CRITIC):
        return True
    if data.batch is not None and "position_ids" in data.batch.keys():
        return data.batch["position_ids"].dim() == 3
    return False


def vl_position_ids_to_text(position_ids: torch.Tensor, attention_mask: torch.Tensor) -> torch.Tensor:
    """Convert Qwen-VL mrope position_ids (4, B, L) or (B, 4, L) to standard text (B, L)."""
    if position_ids.dim() != 3:
        return position_ids
    # mrope stacks text row first, then 3 vision rows (see rollout_loop / rl_dataset).
    if position_ids.size(0) == 4:
        return position_ids[0]
    if position_ids.size(1) == 4:
        return position_ids[:, 0, :]
    # Fallback: standard cumulative positions from attention_mask.
    return compute_position_id_with_mask(attention_mask)


def batch_vl_to_text_for_critic(data):
    """
    Build a text-only DataProto view for critic forward/update.

    - Drops multi_modal_inputs / multi_modal_data from non_tensor_batch.
    - Converts mrope position_ids to (B, L) for causal LM critics.
    - Does not mutate the input batch (actor rollout keeps VL fields).
    """
    from verl import DataProto

    if not is_vl_batch_for_critic(data):
        return data

    batch = data.batch.clone()
    attention_mask = batch["attention_mask"]
    batch["position_ids"] = vl_position_ids_to_text(batch["position_ids"], attention_mask)

    non_tensor_batch = {k: v for k, v in data.non_tensor_batch.items() if k not in VL_NON_TENSOR_KEYS_FOR_CRITIC}

    return DataProto(batch=batch, non_tensor_batch=non_tensor_batch, meta_info=data.meta_info)
